...
from nn.lstmae import DualLSTMAutoencoder, LSTMAutoencoder, MLP
...
def training_fun(args):
    ...
    fmri_seq_len = 30
    model_fmri = LSTMAutoencoder(fmri_seq_len, input_size, hidden_size, num_layers).to(device)
    optimizer_fmri = Adam(model_fmri.parameters(), lr=lr)  

    mrg_seq_len = 240
    model_mrg = LSTMAutoencoder(mrg_seq_len, input_size, hidden_size, num_layers).to(device) 
    optimizer_mrg = Adam(model_mrg.parameters(), lr=lr)  

    mlp_pn = MLP(input_dim = hidden_size, device = device).to(device)  
    optimizer_mlp = Adam([{'params': model_fmri.encoder.parameters(), 'lr': lr},
                          {'params': model_mrg.encoder.parameters(), 'lr': lr},
                          {'params': mlp_pn.parameters(), 'lr': lr}
                        ])
    ...
    for epoch in range(1, args.n_epochs + 1): 
        ...
        # TODO the dada loader should be changed for pn loss 
        for i, (xm, xf, time, subj) in enumerate(tqdm(dataloader)): 
            # import pdb; pdb.set_trace()
            print(subj)  
            
            xf, xm = Variable(xf).squeeze(1).float().to(device), Variable(xm).float().squeeze(1).to(device)

            xf_hat, hf, zf = model_fmri(xf)
            loss_f = loss_function(xf_hat, xf) 
            loss_f.backward()
            optimizer_fmri.step()

            xm_hat, hm, zm = model_mrg(xm)
            loss_m = loss_function(xm_hat, xm)
            optimizer_mrg.zero_grad()
            loss_m.backward()
            optimizer_mrg.step()
 
            # [loss_pn, acc_pn], [zm_mlp, zf_mlp] = mlp_pn(zm, zf)
            ...
            if epoch%10==0:
                data_embeddings[subj] = [time, zm_mlp.detach().cpu().numpy(), zf_mlp.detach().cpu().numpy()]
 
        prt_text =  '     ep.%d       l(p/f/m): %4.2f/%4.2f/ %4.2f || ' 
        ...
        save_list_to_txt('./mar24-0950-farnam', result_list)
        tlf, tlm, tlpn  = Averager(), Averager(), Averager()   
        if epoch%10==0:
